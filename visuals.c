...
//    vec4_t radarcolor = { 0.2f, 0.9f, 0.2f, 0.25f };
	vec4_t colorRedR = { 0.7f, 0.7f, 0.7f, 0.50f };
	...
	drawFillRect(radar2win.posX-((scale/4)*3), radar2win.posY, 1, scale, colorRedR);
	drawFillRect(radar2win.posX-(scale/4), radar2win.posY, 1, scale, colorRedR);
	drawFillRect(radar2win.posX-scale, radar2win.posY + scale/4, scale, 1, colorRedR);
	drawFillRect(radar2win.posX-scale, radar2win.posY + (scale/4)*3, scale, 1, colorRedR);
	drawFillRect(radar2win.posX-(scale/2), radar2win.posY , 1, scale, colorRed);
	drawFillRect(radar2win.posX-scale, radar2win.posY + scale/2, scale, 1, colorRed);
...
void drawSpectators() {
	...
	#define SPECBOX_BORDERCOLOR colorGreen
	...
	for (count = 0; count < seth.specCount; count++) {
		drawText(SPECBOX_SPACE + specwin.posX, y, SPECBOX_FONT_SIZE, seth.specNames[count], colorGreen, qfalse, qtrue, &eth.fontVariable);
		y += lineHeight;
	}
...
void drawIrcFireteam() {
	...
	#define IRCFT_BORDERCOLOR colorGreen
	...
	for (count = 0; count < (sizeof(ircBuddies) / sizeof(ircBuddy_t)); count++) {
		...
		if (buddy->infoValid) {
			...
			if (isOnServer) {
				...
				if (nicks)
					drawText(ircFTwin.posX + IRCFT_SPACE + max, y, IRCFT_FONT_SIZE, buddy->ftString, colorGreen, qfalse, qtrue, &eth.fontVariable);
			}
			drawText(IRCFT_SPACE + ircFTwin.posX, y, IRCFT_FONT_SIZE, buddy->nick + sizeof(IRC_BOT_PREFIX) - 1, isOnServer ? colorGreen : colorGreen, qfalse, qtrue, &eth.fontVariable);
			y += lineHeight;
		}
	}
...
void drawIrcChat() {
	...
	#define IRC_CHAT_TEXT_COLOR colorGreen
	...
	if (ircMsgsGeneral.show) {
		...
		drawRect(ircchatwin.posX, ircchatwin.posY, ircchatwin.windowW, ircchatwin.windowH, colorGreen, IRC_CHAT_BOX_BORDER_SIZE);
...
void drawIrcTopic() {
	...
	Vector4Copy(colorBlue, irctopicwin.bgColor);
	...
	drawTextRect(irctopicwin.posX, irctopicwin.posY, IRCTOPIC_SIZE, str, colorGreen, qfalse, qtrue, qtrue, qtrue, irctopicwin.bgColor, colorRed, &eth.fontVariable);
...
void drawEspEntities() {
	...
	for (entityNum = 0; entityNum < MAX_CLIENTS; entityNum++) {
		...
		if (seth.value[VAR_ESPNAME]) {
			...
			// Define name color	
			vec4_t nameColor;
			Vector4Copy(colorGreen, nameColor);
			nameColor[2] = fadeColor;

			if (seth.value[VAR_ESPNAME_BG]) {
				// Background color
				vec4_t bgColor;
				Vector4Copy(colorRed, bgColor);
				bgColor[3] = fadeColor;
				// Border color
				vec4_t borderColor;
				Vector4Copy(colorGreen, borderColor);
...
void drawSpawnTimerRight() {
    ...
    drawText(x, y + 3, 0.19f, str, colorCyan, qtrue, qfalse, &eth.fontVariable);
}
...
void initHUD() {
	VectorCopy(colorGreen,fxphase.color[0]);
...
void drawETHhud() {
    ...
    for (i=0 ; i < 5 ; i++)  {
		if(strlen(console_p[consoleQueue[i]]) > 0)
			drawText(TXT_X, TXT_Y + (i * 10), 0.19f, console_p[consoleQueue[i]], colorGreen,qfalse, qfalse, &eth.fontVariable);
    }

    for (i=0 ; i < 3 ; i++)  {
		if(strlen(chat_p[chatQueue[i]]) > 0)
			drawText(CHAT_X, CHAT_Y + (i * 10), 0.19f, chat_p[chatQueue[i]], colorGreen,qfalse, qtrue, &eth.fontVariable);
    }
...
void drawPlayerList() {
	...
	for (; catCount < PLIST_MAX; catCount++) {
		...
		// Draw category
		drawFillRect(MENU_SCREEN_BORDER, y, maxItemWidth, itemHeight, isMouseOn ? colorCyan : gameBackColor);
		drawRect(MENU_SCREEN_BORDER, y, maxItemWidth, itemHeight, isMouseOn ? gameBackColor : gameBorderColor, 1);
		drawText(MENU_SCREEN_BORDER + (itemHeight / 2), y + 2, MENU_TEXT_SIZE, plistCats[catCount].name, isMouseOn ? gameFontColor : colorGreen, qfalse, qfalse, &eth.fontVariable);
		...
		for (; client < MAX_CLIENTS; client++) {
			...
			// Draw client
			drawFillRect(X, y, maxItemWidth, itemHeight, isMouseOn ? colorCyan : gameBackColor);
			drawRect(X, y, maxItemWidth, itemHeight, isMouseOn ? gameBackColor : gameBorderColor, 1);
			drawText(X + (itemHeight / 2) + sizeTextWidth(MENU_TEXT_SIZE, " ", &eth.fontVariable), y + 2, MENU_TEXT_SIZE, eth.clientInfo[client].name, colorGreen, qfalse, qfalse, &eth.fontVariable);
...
void drawReferees() {
	if (seth.value[VAR_REFLIST] == REF_LIST) {
		...
		#define REF_SPACE 5
		#define REF_BORDERCOLOR colorGreen
		#define REF_TEXT "Refs:"
		...
		for (count = 0; count < seth.refCount; count++) {
				drawText(REF_SPACE + refwin.posX, y, REF_FONT_SIZE, seth.refNames[count], colorGreen, qfalse, qtrue, &eth.fontVariable);
				y += lineHeight;
		}
		...
		char str[64];
    		snprintf(str, sizeof(str), "Refs: ^2%i", seth.refCount);
   		int w = sizeTextWidth(0.19f, str, &eth.fontVariable);
    		int y = 137;
    		int x = 634 - w;
...
void drawCrosshair() {
	if (seth.value[VAR_CROSSHAIR] == XHAIR_TCE && eth.mod->type != MOD_TCE)
		return;
	drawFillRect(319.5, 235.0, 1, 10, colorLtGrey);
	drawFillRect(315.0, 239.5, 10, 1, colorLtGrey);
}
...
void drawMemDump( int offset ) {
	...
	#define DBOX_BORDERCOLOR colorCyan
	#define DBOX_TEXT ""
	...
	drawText(DBOX_X + DBOX_SPACE, y, DBOX_FONT_SIZE, d_text, colorCyan, qfalse, qtrue, &eth.fontVariable);
	y += lineHeight;
	...
	drawText(DBOX_X + DBOX_SPACE, y, DBOX_FONT_SIZE, DBOX_TEXT_B, colorCyan, qfalse, qtrue, &eth.fontVariable);
}
...
void drawPredictStats(vec3_t origin, vec3_t real, vec3_t predict) {
	...
	for (; count < 3; count++) {
		// Draw background
		drawFillRect(x, PSB_Y, PSB_BAR_WIDTH, PSB_BAR_HEIGHT, colorRed);
		...
		// Draw border
		drawRect(x, PSB_Y, PSB_BAR_WIDTH, PSB_BAR_HEIGHT, colorGreen, 1);
	...
	drawText(PSB_X, PSB_Y + PSB_BAR_HEIGHT + PSB_SPACE, 0.19f, str, colorGreen, qfalse, qtrue, &eth.fontVariable);	
...
void drawStats() {
    ...
    char stats[628];
    sprintf(stats, "^1ETH: ^3v%s ^2|^7Name: ^x%s^* ^2| ^3%s ^2| ^7Kills: ^1%i ^2| ^7Spree: ^1%i ^2| ^7Xp: ^1%d ",         ETH_VERSION, eth.clientInfo[eth.cg_clientNum].name, gameMod, eth.killCount, eth.killCountNoDeath, eth.cg_snap->ps.stats[STAT_XP]); 

	
    int w = eth_CG_Text_Width_Ext(stats, 0.27f, 0, &eth.fontVariable);
    drawFillRect(8, 8, w + 9, 16 + 2, backColor);
	eth_CG_Text_Paint_Ext(10, 10, 0.27f, 0.27f, fontColor, stats, 0, 0, 0, &eth.fontVariable);
}
// DEBUG
