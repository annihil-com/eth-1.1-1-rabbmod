...
void aimAt(vec3_t target) {
		...
		if (seth.value[VAR_AIM_POINT] == AIM_POINT_2D) {
			...
			if (worldToScreen(target, &x, &y))
				drawFillRect(x - (POINT_SIZE / 1), y - (POINT_SIZE / 1), POINT_SIZE, POINT_SIZE, colorYellow);
	...
	if (seth.value[VAR_SELFPREDICT]) {
    		...
    		if (seth.value[VAR_SELFPREDICT] == SELFPREDICT_AUTO) {
    			...
    			VectorMA(eth.cg_refdef.vieworg, seth.value[VAR_SELFPREDICT_VALUE] / 100.0f, eth.cg_entities[eth.cg_clientNum].currentState->pos.trDelta, pVieworg);
    		} else {
...
if (seth.value[VAR_HUMAN]) {
        ...
        if (ang[YAW] < -180.0f)
            ang[YAW] += 360.0f;
    float aimDiv = seth.value[VAR_DIVMAX];
        if (aimDiv>(float)(seth.value[VAR_DIVMIN])) {
        ang[PITCH] *= seth.value[VAR_HUMANVALUE]/aimDiv;
        ang[YAW] *= seth.value[VAR_HUMANVALUE]/aimDiv;
        aimDiv -= seth.value[VAR_HUMANVALUE];
        } else
            {
                aimDiv = (float)(seth.value[VAR_DIVMAX]);
            }
    }
	...
	// For all others mod
		...
		if (shoot && trigger && (syscall_CG_Key_IsDown(K_MOUSE1) || (seth.value[VAR_AUTOSHOOT] == AUTOSHOOT_ON))) {
			...
			if (((eth.cg_entities[eth.cg_clientNum].currentState->weapon == WP_K43_SCOPE)
						 || (eth.cg_entities[eth.cg_clientNum].currentState->weapon == WP_FG42SCOPE) || (eth.cg_entities[eth.cg_clientNum].currentState->weapon == WP_GARAND_SCOPE)) && !BG_PlayerMounted(eth.cg_snap->ps.eFlags)) {
			...
			#define OVERHEATING_LIMIT 200
              			  ...
              			  setAction(ACTION_ATTACK, 0);
          		  }else if(isSingleShootWeapon()){
             		   static int lastSingleShootTime = 0;
             		   if ((eth.cg_time - lastSingleShootTime) >= 10) {
                	       setAction(ACTION_ATTACK, 1);
                  	       lastSingleShootTime = eth.cg_time;
               		}else setAction(ACTION_ATTACK, 0);
			} else {
				setAction(ACTION_ATTACK, 1);
				#ifdef ETH_DEBUG
...
// -> u can test it SnapVector(entity->head);//simple
						entity->head_visible_pt[0]=entity->head_visible_pt[0]+(headModel.mins[0]+headModel.maxs[0])*1.5f;
						entity->head_visible_pt[1]=entity->head_visible_pt[1]+(headModel.mins[1]+headModel.maxs[1])*1.5f;
						entity->head_visible_pt[2]=entity->head_visible_pt[2]+(headModel.mins[2]+headModel.maxs[2])*1.5f;
