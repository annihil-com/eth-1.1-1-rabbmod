...
// Draw the spycam
    ...
    ethEntity_t* entity = &eth.entities[satchelEntityNum];
    char str[128];
    sprintf(str, "SatchelCam");
    vec4_t color={1, 1, 0, 1};
    drawText(385 + (250 - sizeTextWidth(0.19f, str, &eth.fontFixed)) / 2, -15 + 250 - 10, 0.19f, str, colorWhite, qfalse, qtrue, &eth.fontFixed);
    drawText(478, 206, 0.19f, "Reach:", colorWhite, qfalse, qtrue, &eth.fontFixed);
    if(entity->distance>2000)
        drawText(480, 215, 0.19f, "Out of reach", colorRed, qfalse, qtrue, &eth.fontFixed);
    else{
        drawFillRect(480, 215, entity->distance/13, 6, color);
        drawRect(480, 215, 154, 7, colorRed, 1);
    }
}
...
void drawTargCam() {
	...
	// impact
	trace_t targTrace;
	eth.CG_Trace(&targTrace, eth.cg_refdef.vieworg, NULL, NULL, aheadPos, eth.cg_snap->ps.clientNum, CONTENTS_SOLID | CONTENTS_BODY | CONTENTS_ITEM);
	...
	// angle
	AnglesToAxis(eth.cg_refdefViewAngles, camRefDef.viewaxis);
	VectorCopy(targTrace.endpos, camRefDef.vieworg);
	// origin
	vec3_t stepBack;
	VectorMA(vec3_origin, 600, forward, stepBack);
	VectorSubtract(camRefDef.vieworg, stepBack, camRefDef.vieworg);
	
	// Draw the targcam
	drawSpyCam(475, 200, 160, 160, &camRefDef, eth.enemyVisibleShader, eth.friendVisibleShader, qtrue);

char str[128];
	sprintf(str, "Noob cam");
