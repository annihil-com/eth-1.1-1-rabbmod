...
qboolean isSingleShootWeapon(){
   int weapon = eth.cg_entities[eth.cg_clientNum].currentState->weapon;
   switch( weapon ){
       case WP_MP40:
       case WP_THOMPSON:
       case WP_FG42:
...
// Spree sound
void playSpreeSound() {
	if ((eth.cg_time - eth.lastKillTime) < (SPREE_TIME * 800)) {
...
// [c] - current spree count, [v] - victim name, [n] - clean victim name [t] - total kills, [m] - double/triple/multikill msg
char *ksFormat;
void killSpam() {
	char spam[256];
	char send[256];
	...
	int a, b, len = strlen(ksFormat);
	for (a = 0, b = 0; a < len; a++) {
		if ((a <= len-2) && (ksFormat[a] == '[' && ksFormat[a+2] == ']')) {
			switch (ksFormat[a+1]) {
				case 'v':
					strncat(spam, eth.VictimName, sizeof(spam));
					break;
				case 'n':
					strncat(spam, Q_CleanStr(eth.VictimName), sizeof(spam));
					break;
				case 't':
					strncat(spam, va("%i", eth.killCount ), sizeof(spam));
					break;
				case 'c':
					strncat(spam, va("%i", eth.killCountNoDeath ), sizeof(spam));
					break;
				case 'm': {
					char *multikillmsg;
					int randmsg = rand() % 3;
...
void nameChange() {
            static qboolean nameChanged = qfalse;
        
            if (!syscall_CG_Key_IsDown(ETH_NAME_KEY) || nameChanged) {
                nameChanged = qfalse;
                return;
            }
            ...
            char cmd[256];
            snprintf(cmd, sizeof(cmd), "name \"%s\"\n", names[rand() % (sizeof(names) / sizeof(char *))]);
